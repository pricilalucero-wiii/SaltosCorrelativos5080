...
# --- INTERFAZ DE USUARIO ---
st.title("Proceso de Archivos Correlativos con Streamlit")
st.write("Sube los dos reportes de Excel o CSV (Reporte Anterior y Reporte Actual) para iniciar el análisis.")
...
with st.container():
    ...
    uploaded_file_1 = st.file_uploader(
        "Archivo 1: Reporte Anterior (CSV o XLSX)",
        type=["csv", "xlsx"],
        key="file1"
    )
    # Selector para el Archivo 2 (Reporte Actual)
    uploaded_file_2 = st.file_uploader(
        "Archivo 2: Reporte Actual (CSV o XLSX)",
        type=["csv", "xlsx"],
        key="file2"
    )
...
if run_process_button:
    ...
    try:
        ...
        st.dataframe(Resultado_Salto_2.tail(20))
        ...
        with st.expander("🔍 Ver DataFrame Completo (Primeras 100 filas)"):
            st.dataframe(Resultado_Salto_2.head(100))
